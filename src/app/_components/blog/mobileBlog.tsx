    ...
    return (
        <div className="w-full px-4 py-8">
            <div className="text-center mb-8">
                ...
                <h4 className="text-sm text-gray-500">Stay informed with our latest news and insights</h4>
            </div>

            <div className="flex flex-col items-center w-full">
                {/* Blog post card */}
                <div 
                    className="w-full bg-[#2121218f] rounded-lg overflow-hidden mb-6 cursor-pointer transition-all duration-300 transform hover:scale-[1.02] hover:shadow-lg"
                    onClick={() => window.location.href=blogPosts[activeIndex].url}
                >
                    <div className="relative w-full h-96">
                        <ImageVideoLoader
                            ...
                            width={400}
                            height={800}
                            className={`object-cover h-full ${imageOpacity} transition-transform duration-300 hover:scale-105`}
                        />
                    </div>
                    <div className="p-4">
                        <h4 className="text-gray-500 text-sm mb-2">{blogPosts[activeIndex].date}</h4>
                        <h3 className="text-xl font-bold mb-2 group-hover:text-[var(--text-highlight)] transition-colors duration-300">{blogPosts[activeIndex].title}</h3>
                        <p className="text-gray-400 text-sm mb-4">{blogPosts[activeIndex].excerpt}</p>
                        <button 
                            className="text-[var(--text-highlight)] hover:text-[var(--text-highlight-hover)] transition-all duration-300 text-sm hover:translate-x-2 inline-flex items-center"
                            onClick={(e) => {
                                e.stopPropagation();
                                window.location.href="/blog/" + blogPosts[activeIndex].id;
                            }}
                        >
                            Read More →
                        </button>
                ...
                <div className="flex justify-between w-full">
                    ...
                    <button
                        onClick={() => handleStepChange(Math.min(blogPosts.length - 1, activeIndex + 1))}
                        disabled={activeIndex === blogPosts.length - 1}
                        className={`px-4 py-2 text-sm rounded-md ${
                            activeIndex === blogPosts.length - 1 ? 'text-gray-300' : 'text-[var(--text-highlight)]'
                        }`}
                    >
                        Next
                    </button>
